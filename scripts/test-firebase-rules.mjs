...
import { getDownloadURL, listAll, ref as storageRef, uploadString } from 'firebase/storage';
...
try {
    ...
    await assertFails(
        setDoc(doc(collection(publicDb, 'comprovantes')), {
            instituicao: 'bradesco',
    ...
    await assertFails(
        setDoc(doc(collection(publicDb, 'acessos')), {
            comprovanteId,
            ownerUid: 'other-uid',
            at: serverTimestamp(),
        }),
    ...
    const photoPath =
        'capturas/owner-uid/' + comprovanteId + '/' + acessoEvidenciaRef.id + '/photo.jpg';
    await assertSucceeds(
        uploadString(
            storageRef(publicStorage, photoPath),
